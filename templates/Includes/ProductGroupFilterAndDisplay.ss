...
<% if HasFilters %>
	...
	<div class="filterForSection">
		<ul id="FilterForList" class="filterOptions filterSortOptions">
			<% if FilterLinks %><% loop FilterLinks %><li class="$FirstLast standardFilters $AjaxDefinitions.ProductListAjaxifiedLinkClassName"><a href="$Link" class="$LinkingMode">$Name<% if First %><% else %> ($Count)<% end_if %></a></li><% end_loop %><% end_if %>
...
<% if DisplayLinks %>
	<h3>
		...
		<div class="displayStyleSection $AjaxDefinitions.ProductListAjaxifiedLinkClassName">
