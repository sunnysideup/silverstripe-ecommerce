...
class EcommerceProductImageReset extends BuildTask{
	...
	function run($request){
		...
		foreach($tables as $tableName) {
			...
			if($removeCount) {
				DB::alteration_message("Removed $removeCount image(s) from products and variations because they do not exist in the file-system or database", "deleted");
			}
			else {
				DB::alteration_message("All product images are accounted for", "created");
			}
			if($updateClassCount) {
				DB::alteration_message("$removeCount image(s) did not match the requirement 'instanceOF Product_Image', this has been corrected.", "deleted");
			}
			else {
				DB::alteration_message("All product images instancesOF Product_Image", "created");
			}
		}
	}
