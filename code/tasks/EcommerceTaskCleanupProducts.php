...
class EcommerceTaskCleanupProducts extends BuildTask
{
    ...
    public function run($request)
    {
        ...
        DB::alteration_message('<h3>Cleaning up ProductGroup Product links</h3>', 'created');
        DB::query('INSERT IGNORE INTO Product_ProductGroups (ProductID, ProductGroupID) Select ID, BrandID from PhotographicProduct ');
        DB::query('CREATE TEMPORARY TABLE Product_ProductGroups_temp(ProductID INT, ProductGroupID INT) ; ');
        DB::query('INSERT INTO Product_ProductGroups_temp(ProductID, ProductGroupID) SELECT DISTINCT ProductID, ProductGroupID FROM Product_ProductGroups; ');
        DB::query('DELETE FROM Product_ProductGroups; ');
