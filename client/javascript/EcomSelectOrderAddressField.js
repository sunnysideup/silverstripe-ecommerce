  ...
  $(document).ready(function () {
    if (typeof EcomSelectOrderAddressFieldOptions !== 'undefined') {
      for (var i = 0; i < EcomSelectOrderAddressFieldOptions.length; i++) {
        EcomSelectOrderAddressField.set_data(
          EcomSelectOrderAddressFieldOptions[i].id,
          EcomSelectOrderAddressFieldOptions[i].address
        )
      }
...
var EcomSelectOrderAddressField = {
   ...
   */
  data: [],
  set_data: function (i, object) {
    EcomSelectOrderAddressField.data[i] = object
  },
  ...
  setupAddressChanges: function () {
    window
      .jQuery(EcomSelectOrderAddressField.fieldSelector)
      .each(function (i, el) {
        //window.jQuery(el).next(EcomSelectOrderAddressField.addressSelector).hide();
        window
          .jQuery(el)
          .find(EcomSelectOrderAddressField.inputSelector)
          .each(function (i, el) {
            window.jQuery(el).change(function (e) {
              ...
              if (data) {
                window.jQuery.each(data, function (i, n) {
                  window
                    .jQuery(
                      "input[name='" +
                        i +
        ...
        window.jQuery(this).find('input:first').trigger('click')
      })
  },

  setupNoLongerInUseLinks: function () {
    window
      .jQuery(EcomSelectOrderAddressField.removeLinkSelector)
      .click(function (e) {
        e.preventDefault()
        ...
        window.jQuery.get(url, function (data) {
          ...
          window
            .jQuery(this)
            .removeClass(EcomSelectOrderAddressField.loadingClass)
        })
      })
  }
}
