  ...
  const EcomCheckoutPage = {
    ...
    makeAllPreviousAndNextAjaxified: function () {
      window
        ...
        .on(
          'click',
          EcomCheckoutPage.nextPreviousOverviewListSelector +
            ', ' +
            EcomCheckoutPage.nextPreviousButtonsSelector,
          function (event) {
            event.preventDefault()
            var href = window.jQuery(this).attr('href')
            window
              .jQuery(EcomCheckoutPage.outerHolderSelector)
              .fadeOut(function () {
                window
                  .jQuery(EcomCheckoutPage.outerHolderSelector)
                  .addClass(EcomCheckoutPage.loadingClass)
                var jqxhr = window.jQuery
                  .ajax({
                    url: href,
                    settings: {
                      cache: false
                  ...
                  .always(function () {
                    window
                      ...
                      .fadeIn(function () {
                        window
                          .jQuery(EcomCheckoutPage.outerHolderSelector)
                          .removeClass(EcomCheckoutPage.loadingClass)
                      })
                  })
              })
            return false
    ...
    attachCSSAndJSHeader: function (jqXHRResponse) {
      var headers = EcomCheckoutPage.parseResponseHeaders(
        jqXHRResponse.getAllResponseHeaders()
      )
      ...
      if (CSSArray.length > 0) {
        window.jQuery.each(CSSArray, function (index, value) {
          window
            .jQuery('<link>')
            .appendTo('head')
      ...
      if (JSArray.length > 0) {
        window.jQuery.each(JSArray, function (index, value) {
