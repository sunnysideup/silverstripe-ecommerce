...
const EcomBuyableSelectField = {
  ...
  */
  ...
  attach: function () {
    ...
    window.jQuery('#FindBuyable input.text')
    .focus(
      function () {
        var labelSelector = "label[for='" + window.jQuery(this).attr('id') + "']"
    ...
    .autocomplete(
      {
        ...
        source: function (request, response) {
          window.jQuery("label[for='" + EcomBuyableSelectField.fieldName + "']'").parent().addClass(EcomBuyableSelectField.loadingClass)
          EcomBuyableSelectField.requestTerm = request.term
          window.jQuery.ajax(
            {
              type: 'POST',
              url: window.jQuery('base').attr('href') + EcomBuyableSelectField.url_segment + '/json/',
              dataType: 'json',
              data: {
                term: request.term,
              ...
              success: function (data) {
                ...
                window.jQuery("label[for='" + EcomBuyableSelectField.fieldName + "']'").parent().removeClass(EcomBuyableSelectField.loadingClass)
              }
            }
          )
        },
        ...
        select: function (event, ui) {
          var formSelector = '#' + window.jQuery('#FindBuyable').parents('form').attr('id')
          if (EcomBuyableSelectField.formName) {
            formSelector = '#' + EcomBuyableSelectField.formName
          }
          if (
            window.jQuery(formSelector + " input[name='BuyableID']").length == 0 ||
            window.jQuery(formSelector + " input[name='BuyableClassName']").length == 0 ||
            ...
            window.jQuery(formSelector + " input[name='BuyableID']").val(ui.item.id)
            window.jQuery(formSelector + " input[name='BuyableClassName']").val(ui.item.className)
            window.jQuery(formSelector + " input[name='Version']").val(ui.item.version)
  ...
  showCurrentSituation: function (situation) {
    window.jQuery("input[name=\'" + EcomBuyableSelectField.selectedBuyableFieldName + "\']").val(situation)
    window.jQuery('#FindBuyable span#FindBuyable-SelectedBuyable').text(situation)
  }
