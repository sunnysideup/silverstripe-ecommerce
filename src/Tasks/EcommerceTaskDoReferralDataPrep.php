...
class EcommerceTaskDoReferralDataPrep extends BuildTask
{
    ...
    public function doDataPrep(?int $limit = 9999999, ?int $start = 0, ?bool $retainMessages = false): bool
    {
    ...
    protected function recalculateReferrals(int $limit, int $start)
    {
        ...
        $daysAgoStale = (int) Config::inst()->get(self::class, 'recalculate_days_for_prep_data') ?: (180);
        $refs = Referral::get()
            ->filter(['Processed' => 0])
            ->sort('ID', 'ASC')
            ->limit($limit, $start);
