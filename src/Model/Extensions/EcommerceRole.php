...
class EcommerceRole extends Extension implements PermissionProvider, PermissionProviderFactoryProvider
{
    ...
    public static function get_customer_group()
    {
        ...
        return DataObject::get(Group::class)->setUseCache(true)->filter(['Code' => $customerGroupCode])->first();
    }
    ...
    public static function list_of_admins($showUnselectedOption = false)
    {
        ...
        $group = DataObject::get(Group::class)->setUseCache(true)->filter(['Code' => 'administrators'])->first();
        //fill array
        if ($group) {
            $members = $group->Members();
            $membersCount = $members->count();
    ...
    public static function get_admin_group()
    {
        ...
        return DataObject::get(Group::class)->setUseCache(true)->filter(['Code' => $adminCode])->first();
    }
    ...
    public static function get_assistant_group()
    {
        ...
        return DataObject::get(Group::class)->setUseCache(true)->filter(['Code' => $assistantCode])->first();
    }
    ...
    public function previousUnsubmittedOrders(int $currentOrderID, ?int $firstStepID = 0)
    {
        $owner = $this->getOwner();
        if (! $firstStepID) {
            $firstStepID = DataObject::get(OrderStep::class)->setUseCache(true)->first()->ID;
        }
    ...
    public function LastPurchaseTs(): ?int
    {
        $successfulPayments = DB::query(
            "SELECT \"EcommercePayment\".\"Created\"
                FROM \"EcommercePayment\"
                INNER JOIN \"Order\"
                ON \"EcommercePayment\".\"OrderID\" = \"Order\".\"ID\"
                WHERE
        ...
        if ($successfulPayments->numRecords()) {
