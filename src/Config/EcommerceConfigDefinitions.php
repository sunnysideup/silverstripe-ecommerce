...
class EcommerceConfigDefinitions
{
    ...
    * ### @@@@ STOP REPLACEMENT @@@@ ###
    */
    public function Definitions($className = '',
    $variable = '')
    {
        $array = [
            ################### GENERAL AND CMS CONFIG #####################
            ...
            'dialogue_js_file_location' => 'The location for the dialogue (pop-up) javascript.  E-commerce comes with it a default <i>Simple Dialogue</i> pop-up dialogue,
            ...
            when for example,
            you are deciding whether or not products can be sold based on the country of the customer.',
            'maximum_number_of_products_to_list' => 'The maximum number of products to be shown in a list.  For performance reasons,
            we suggest you limit this to 1000 for big sites and as low as 200 for small servers.',
            'sort_options' => "associative sort options array with sub-keys of Title and SQL,
            e.g. 'default' = array('Title' => 'default',
            'SQL' => 'Title DESC')",
            'filter_options' => "associative filters options array with sub-keys of Title and SQL,
            e.g. 'default' = array('Title' => 'Featured',
            'SQL' => 'Featured = 1')",
            'display_styles' => "associative display styles array with its key as template name,
            e.g. 'MyTemplateName' => 'Full Details'",
            ...
            'ProductGroupSearchPage' => ['best_match_key' => 'Sort key used to sort by relevance.',
            ...
            'ShoppingCart' => ['session_code' => 'The code use for the session variable that stores the Order ID.',
            ...
            'CheckoutPage_Controller' => ['checkout_steps' => 'The Checkout Steps.  This can be defined as you like,
            but the last step should always be: orderconfirmationandpayment.',
            'ajaxify_steps' => 'Array of Javascript files that are required to ajaxify the steps in the checkout. Defaults to none,
            but there is a sample JS file available: ecommerce/javascript/EcomCheckoutPage.js.'],

            'ShopAccountFormValidator' => ['minimum_password_length' => 'The minimum length of the password for an account.'],

            'OrderModifierForm' => ['controller_class' => 'The controller class is used for Order Modifier Forms.',
            ...
            ################### POST SALE PROCESSING #####################
            'OrderConfirmationPage_Controller' => ['include_as_checkout_step' => 'Include the order confirmation as one of the checkout steps,
            visually,
            in the list of steps shown.',
            ...
            'OrderStepConfirmed' => ['list_of_things_to_check' => 'One of the steps in the order steps sequence is the Order Confirmation.  This is when the Shop Admin looks at all the detail in the order and confirms it is ready to be completed.  Here you can create an HTML list of items to check (e.g. has it been paid,
            do you have the products in stock,
            is there a delivery address,
            etc....)'],

            'OrderStatusLog' => ['available_log_classes_array' => 'Tells us what order log classes are to be used. OrderStatusLogSubmitted should always be used and does not need to be listed here.',
            'order_status_log_class_used_for_submitting_order' => 'This is the log class used to record the submission of the order.  It is crucial to set this to the right log class,
            as a lot of the functionality in e-commerce depends on it: '],

            'OrderStatusLogForm' => ['controller_class' => 'The controller class is used for OrderStatusLogForm forms.',
            'validator_class' => 'The validator class is used for OrderStatusLogForm forms.'],

            'OrderEmail' => ['send_all_emails_plain' => 'Should all the emails be send as plain text?  Not recommended.',
            ...
            ################### ORDER DETAILS #####################
            'Order' => ['modifiers' => 'This is the single most important setting.  here you determine what modifiers are being added to every order.  You can just add them as a non-associative array.  However,
            ...
            'maximum_ignorable_sales_payments_difference' => "The maximum allowable difference between the Order Total and the Payment Total. \tIf this value is,
            for example,
            10 cents and the total amount outstanding for an order is less than ten cents,
            than the order is considered 'paid'",
            'order_id_start_number' => 'The starting number for the order number. For example,
            if you enter 1000 here then the first order will have number 1000,
            the next one 1001 and so on.',
            'template_id_prefix' => 'If you end up with conflicts in your templates (e.g. having the same ID twice) then you can use this variable to set an prefix to all PHP generated IDs in all templates. We use these PHP generated IDs for AJAX templates - where HTML,
            JS and PHP need to work together.',
            ...
            'date_format_for_title' => 'PHP Date format to show date in the title of the order e.g. Y-m-d,
            ...
            ################### CUSTOMERS #####################
            'OrderAddress' => ['use_separate_shipping_address' => 'Do the goods need to get shipped and if so,
            do we allow these goods to be shipped to a different address than the billing address?',
            'use_shipping_address_for_main_region_and_country' => 'In determing the country/region from which the order originated. For,
            for example,
            ...
            'EcommerceRole' => ['permission_category' => 'E-commerce permission group name.',
            ...
            'must_have_account_to_purchase' => 'When this is set to TRUE,
            ...
            'automatically_update_member_details' => 'When set to true,
            ...
            'BillingAddress' => ['allow_selection_of_previous_addresses_in_checkout' => 'In the checkout,
            allow a customer to select from previously used addresses.',
            ...
            'ShippingAddress' => ['allow_selection_of_previous_addresses_in_checkout' => 'In the checkout,
            allow a customer to select from previously used addresses.',
            'required_fields' => 'List of fields that is required to be entered.',
            ...
            'EcommerceCountry' => ['allowed_country_codes' => 'To what countries are you selling?  You can leave this as an empty array,
            ...
            'EcommerceRegion' => ['visitor_region_provider' => 'The class that is being used to provide the region of the customer. It is sort of like a GEOIP for regions.',
            'show_freetext_region_field' => "Provide a free text region field if no regions are specified. Region can also be 'State',
            or 'Province',
            etc..."],

            ################### PAYMENT AND MONEY #####################
            'EcommerceCurrency' => ['default_currency' => 'The default currency used on the site.',
            'exchange_provider_class' => 'The name of the class used to provide currency exchange rate.... You can easily built your own class here that can either provide fixed rates,
            ...
            'EcommercePayment' => ['supported_methods' => 'Associative array of payment methods,
            ...
            ################### CLEANUP AND OTHER TASKS #####################
            'EcommerceTaskCartCleanup' => ['clear_minutes' => 'The number of minutes after which carts are considered abandonned. If set to zero,
            all objects will be cleared. If set to ten,
            objects older than ten minutes will be cleared.',
            'clear_minutes_empty_carts' => 'The number of minutes after which empty carts should be deleted (to reduce the amount of empty (meaningless) carts in the database). If set to zero,
            all objects will be cleared. If set to ten,
            objects older than ten minutes will be cleared.',
            ...
            'never_delete_if_linked_to_member' => 'If set to TRUE,
            then orders with a member linked to it will never be deleted.',
        ...
        $extendedArray = $this->extend('moreDefinitions',
        $array);
