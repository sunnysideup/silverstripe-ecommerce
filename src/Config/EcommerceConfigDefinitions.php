...
class EcommerceConfigDefinitions
{
    ...
    protected $ajaxMethods = [
        ...
        'TotalItemsTimesQuantityClassName' => 'The total number of items times their quantity in the order.
            Use in the following context: AjaxDefinitions.TotalItemsClassNameTimesQuantity',

        'TableID' => 'The main definition on which a lot of others are based.
        ...
        'HiddenPageTitleID' => 'The ID used to identify a (hidden) element that contains the title of the page.
        ...
        'ProductListHolderID' => 'The ID used to identify the product group list holder.
    ...
    public function Definitions($className = '', $variable = '')
    {
        $array = [
            ...
            EcommerceConfigAjax::class => [
                ...
                'cart_js_file_location' => 'The location for the EcomCart.js (javascipt that runs the cart) file.
            ...
            StoreAdmin::class => [
                'managed_models' => 'An array of data object classes that are managed as \'Store\' configuration items.
            ...
            //################## PRODUCT DISPLAY #####################
            ProductGroup::class => [
                'base_buyable_class' => 'The base class for the products being retrieved.
                ...
                'maximum_number_of_products_to_list' => 'The maximum number of products to be shown in a list.
                ...
                'session_name_for_product_array' => 'This is the name for variable stored in session.
            ...
            ProductSearchForm::class => [
                'include_price_filters' => 'For the product search form,
            ...
            ShoppingCart::class => [
                ...
                'cleanup_every_time' => 'Are carts are cleaned up all the time
            ...
            CartResponse::class => [
                'cart_responses_required' => 'An array of the cart responses required for AJAX.
                    This array also identifies the unique IDs used in the html that will be updated by the ajax response.',
            ],

            CartPageController::class => [
                'session_code' => 'Code name for session variable used in Cart Page.
                    This session variable is used to retain a message.',
            ],

            CheckoutPageController::class => [
                'checkout_steps' => 'The Checkout Steps.  This can be defined as you like,
                    but the last step should always be: orderconfirmationandpayment.',

                'ajaxify_steps' => 'Array of Javascript files that are required to ajaxify the steps in the checkout.
            ...
            OrderFormAddress::class => [
                'shipping_address_first' => 'Show the shipping address before the billing address.
            ...
            OrderStep::class => [
                'order_steps_to_include' => 'Another very important definition.
            ...
            OrderStepConfirmed::class => [
                'list_of_things_to_check' => 'One of the steps in the order steps sequence is the Order Confirmation.
            ...
            OrderStatusLog::class => [
                ...
                'order_status_log_class_used_for_submitting_order' => 'This is the log class used to record the submission of the order.
            ...
            //################## ORDER DETAILS #####################
            Order::class => [
                'modifiers' => 'This is the single most important setting.
                ...
                'maximum_ignorable_sales_payments_difference' => 'The maximum allowable difference between the Order Total and the Payment Total.
                ...
                'order_id_start_number' => 'The starting number for the order number.
                    For example, if you enter 1000 here then the first order will have number 1000,
                    the next one 1001 and so on.',

                'template_id_prefix' => 'If you end up with conflicts in your templates (e.g. having the same ID twice)
                ...
                'ajax_subtotal_format' => 'This is used when AJAX returns some values to update on the checkout page.
                ...
                'ajax_total_format' => 'This is used when AJAX returns some values to update on the checkout page.
                    Specify which function returns the Total value. You can also specify if you want a format to be called on that function.',

                'date_format_for_title' => 'Date format to show date in the title of the order e.g. see http://userguide.icu-project.org/formatparse/datetime for details
            ...
            //################## CUSTOMERS #####################
            OrderAddress::class => [
                'use_separate_shipping_address' => 'Do the goods need to get shipped and if so,
                ...
                'field_class_and_id_prefix' => 'In case you have some conflicts in the class / IDs for formfields
            ...
            EcommerceRole::class => [
                ...
                'allow_customers_to_setup_accounts' => 'Allow customers to become members when they purchase items.
            ...
            BillingAddress::class => [
                'allow_selection_of_previous_addresses_in_checkout' => 'In the checkout,  a customer to select from previously used addresses.',
                'required_fields' => 'List of fields that is required to be entered.',
                'fields_to_google_geocode_conversion' => 'This variable tells us how Billing Fields map to the Google Geo Coding objects.
            ...
            ShippingAddress::class => [
                ...
                'fields_to_google_geocode_conversion' => 'This variable tells us how Shipping Fields map to the Google Geo Coding objects.
                    If you set it to null or an empty array then there will be no geocoding.
                    See https://developers.google.com/maps/documentation/geocoding/#Types for more information.',
            ],

            EcommerceCountry::class => [
                'allowed_country_codes' => 'To what countries are you selling?  You can leave this as an empty array,
            ...
            EcommerceCurrency::class => [
                ...
                'exchange_provider_class' => 'The name of the class used to provide currency exchange rate....
            ...
            EcommercePayment::class => [
                'supported_methods' => 'Associative array of payment methods,
            ...
            //################## CLEANUP AND OTHER TASKS #####################
            EcommerceTaskCartCleanup::class => [
                'clear_minutes' => 'The number of minutes after which carts are considered abandonned. If set to zero,
                        all objects will be cleared. If set to ten,
                        objects older than ten minutes will be cleared.',

                'clear_minutes_empty_carts' => 'The number of minutes after which empty carts should be deleted (to reduce the amount of empty (meaningless) carts in the database).
                    If set to zero, all objects will be cleared.
                    If set to ten, objects older than ten minutes will be cleared.',

                'maximum_number_of_objects_deleted' => 'This sets the total number of objects to be cleaned per clean.
                ...
                'one_to_one_classes' => 'An array of key / value pairs that are linked to orders as one-to-one relationships.
                    The key is the order field name (e.g. BillingAddressID) and the value is the class name (e.g. BillingAddress)',

                'one_to_many_classes' => 'An array of key / value pairs that are linked to orders as one-to-many relationships.
                     The key is the class with the order ID and the value is the class name with the LastEdited field.',

                'many_to_many_classes' => 'An array of key / value pairs that are linked to orders as many-to-many relationships.
