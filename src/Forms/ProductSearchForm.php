        ...
        if (isset($data['Keyword']) && $keywordPhrase = $data['Keyword']) {
            if ($baseList->count()) {
                if (strlen($keywordPhrase) > 1) {
                    ...
                    $count = $list1->count();
                    if ($count === 1) {
                        $immediateRedirectLink = $list1->First()->getRequestHandler()->getRequestHandler()->getRequestHandler()->Link();
...
/**
  ...
  */
        ...
        if ($immediateRedirectLink) {
            ...
            $link = $redirectToPage->getRequestHandler()->getRequestHandler()->getRequestHandler()->Link($this->controllerSearchResultDisplayMethod);
        }
